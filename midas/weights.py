...
class ExpAlmonWeights(WeightMethod):
    ...
    def weights(self, nlags):
        ...
        value_to_exp = self.theta1 * ilag + self.theta2 * ilag ** 2
        clipped_value = np.clip(value_to_exp, -np.inf, 700)  # 700 is an arbitrary value, but exp(709) is close to the upper limit for double precision floats
        z = np.exp(clipped_value)
